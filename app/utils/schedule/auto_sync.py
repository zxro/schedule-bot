...
async def _scheduler_loop():
    ...
    3. Ждёт до следующего дня (в 03:00 ночи по локальному времени).
    """

    while True:
        now = datetime.now()
        run_time = datetime(now.year, now.month, now.day, 3, 0, 0)  # 03:00 ночи
        ...
        logger.info(f"🕒 Следующая проверка синхронизации запланирована на {run_time.strftime('%Y-%m-%d %H:%M:%S')}")
...
async def schedule_sync_task():
    ...
    Использование:
        В основном файле приложения (main.py) добавить:
